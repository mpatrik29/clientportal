...
const fetchSubscriptionDetails = async (subscriptionId: string) => {
  try {
    const jwt = await account.createJWT();
    const response = await fetch(`https://68319117000d7a40def3.aw.pure24.co/subscription/details`, {
      method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-appwrite-jwt': jwt.jwt
...
const processFlexiblePayment = async (amount: number) => {
  ...
  useEffect(() => {
    const fetchSubscriptionDetails = async () => {
      ...
      try {
        ...
        setActiveEntryId(summaryData.upcomingPayment?.$id);
  ...
  <div className="w-full md:w-1/5">
    <div className="card shadow-sm bg-white rounded-lg p-6 h-full">
      <h3 className="text-lg font-bold text-gray-800 mb-4">Quick Stats</h3>
      <div className="space-y-4">
        <div>
          <p className="text-sm text-gray-500">Plan Type</p>
          <p className="font-medium">{subscription.plan.planType}</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Lock-in Period</p>
          <p className="font-medium">{subscription.plan.lockinPeriod} months</p>
        </div>
        <div>
          <p className="text-sm text-gray-500">Instrument Period</p>
          <p className="font-medium">{subscription.plan.investmentPeriod} months</p>
        </div>
      </div>
    </div>
  </div>

  {/* Existing Card - 80% width */}
